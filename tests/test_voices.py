...
def test_every_subcommand_parses_and_has_its_dests():
    ...
    while the handler read `args.cookie`, and `schedule` read a `--calendar` flag
    that had been dropped. Both crashed only when actually invoked."""
    import argparse
    import contextlib
    import io
    ...
    names = [
        ...
        ["serve"], ["book"], ["feed"], ["models"], ["login"], ["ui"],
    ]
    for argv in names:
